...
class PlaylistRepositoryImpl(context: Context) : PlaylistRepository {
    ...
    private val playlistDao = AppDatabase.Companion.getInstance(context).playListDao()
    private val crossRefDao = AppDatabase.Companion.getInstance(context).playlistSongCrossRefDAO()
